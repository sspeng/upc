...
int main(){
	...
	shared int * global_T = (shared int *) upc_all_alloc( M*(N_padded+1), sizeof(int) );

	for(j=0; j<M; j++){
		int my_row = 0;
		for(i=0; i<=N_padded; i++)
			if(get_responsible_thread(i, no_elements) == MYTHREAD && my_row < no_elements){
				global_T[i+j*(N_padded+1)] = local_T[my_row + j*no_elements];
				my_row++;
			}
	}
    ...
    if(MYTHREAD == 0){
        ...
        for(i = 0; i<=N_padded; i++){
			printf("%3d | ",i);
        	for(j = 0; j<M; j++)
                printf("%3d ", global_T[i + j*(N_padded+1)]);
	...
	//upc_free( global_T );  // unsure why leaving this in results in segfaults
	free( local_T );
